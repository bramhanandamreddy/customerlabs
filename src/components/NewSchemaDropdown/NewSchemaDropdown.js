...
const NewSchemaDropdown = (props) => {
  ...
  const editNewSchema = (element, elementToBeRemove) => {
    ...
    let a = array.filter(
      (ele) => Object.values(ele)[0] !== Object.values(elementToBeRemove)[0]
    );
    a.push(element);
    setArray(a);
    console.log(array);
      ...
      {array.length !== 0 && (
        <div className="new-schema-container">
          {array.map((element, index) => (
            <>
              <div
                className="schema-dropdown-container"
                style={{ margin: "5px" }}
              >
                <span
                  className="add-schema-bullet"
                  style={{ backgroundColor: `${Object.values(element)[1]}` }}
                ></span>
